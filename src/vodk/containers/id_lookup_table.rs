...
impl IdLookupTable {
    ...
    pub fn add(&mut self) -> Index {
        ...
        *(self.index_to_data.get_mut(idx).unwrap()) = self.data_to_index.len() as Index;
    ...
    pub fn remove(&mut self, idx: Index) {
        ...
        if o == self.data_to_index.len()-1 {
            ...
            *(self.index_to_data.get_mut(moved as uint).unwrap()) = o as Index;
            *(self.data_to_index.get_mut(o).unwrap()) = moved;
        }
        *(self.index_to_data.get_mut(idx as uint).unwrap()) = self.free_list;
        self.free_list = idx;
    }
    ...
    pub fn swap_offsets(&mut self, o1: Index, o2: Index) {
        ...
        *(self.data_to_index.get_mut(o1 as uint).unwrap()) = self.data_to_index[o2 as uint];
        *(self.data_to_index.get_mut(o2 as uint).unwrap()) = temp;
        *(self.index_to_data.get_mut(self.data_to_index[o2 as uint] as uint).unwrap()) = o1;
        *(self.index_to_data.get_mut(self.data_to_index[o1 as uint] as uint).unwrap()) = o2;
    }
}
